...
describe('Kaggle arena bridge', () => {
  ...
  it('applies Slow as an authoritative multi-target control debuff', () => {
    const encounter = new Encounter({ seed: 1 });
    encounter.addCreature({ heroClass: 'Wizard', heroLevel: 5, team: 'red', position: { x: 0, y: 0 }, heroOverrides: { additionalActions: [slow('int', 3, 3)], additionalResources: { 'slot-3': 1 } } });
    encounter.addCreature({ heroClass: 'Fighter', heroLevel: 5, team: 'blue', position: { x: 4, y: 0 } });
    encounter.start();
    const caster = encounter.state!.creatures.find(creature => creature.team === 'red')!;
    const target = encounter.state!.creatures.find(creature => creature.team === 'blue')!;
  ...
  it('ends Web restraint when its concentration is dropped', () => {
    ...
    expect(encounter.state!.persistentZones).toHaveLength(1);
    target.conditions = [];
    target.activeBuffs = [];
    withRng({ next: () => 0 }, () => processTargetTurnStartOngoingEffects(encounter.state!, target));
    expect(target.conditions).toContain('restrained');
  ...
  it('reapplies Entangle when a creature enters its persisted area', () => {
    ...
    const caster = encounter.state!.creatures.find(creature => creature.team === 'red')!;
    const target = encounter.state!.creatures.find(creature => creature.team === 'blue')!;
    withRng({ next: () => 0 }, () => expect(executeSpell(encounter.state!, caster, entangle('wis', 3, 3), target, [target], target.position)).toBe(true));
    target.conditions = [];
    target.activeBuffs = [];
    withRng({ next: () => 0 }, () => triggerPersistentZones(encounter.state!, target, 'entry'));
    expect(target.conditions).toContain('restrained');
  });
  ...
  it('makes Invisibility hide its target and end on an attack', () => {
    const encounter = new Encounter({ seed: 1 });
    encounter.addCreature({ heroClass: 'Wizard', heroLevel: 5, team: 'red', position: { x: 0, y: 0 }, heroOverrides: { additionalActions: [invisibility('int', 3, 3)], additionalResources: { 'slot-2': 1 } } });
    encounter.addCreature({ monster: 'Ogre', team: 'blue', position: { x: 1, y: 0 } });
    encounter.start();
    const caster = encounter.state!.creatures.find(creature => creature.team === 'red')!;
    const target = encounter.state!.creatures.find(creature => creature.team === 'blue')!;
    expect(executeSpell(encounter.state!, caster, invisibility('int', 3, 3), caster)).toBe(true);
    expect(caster.conditions).toContain('invisible');
    expect(canSee(encounter.state!, target, caster)).toBe(false);
    const attack = caster.monsterData.actions.find(action => action.type === 'melee' && action.attackBonus !== undefined)!;
    resolveAttack(encounter.state!, caster, target, attack);
    expect(caster.conditions).not.toContain('invisible');
    expect(canSee(encounter.state!, target, caster)).toBe(true);
  });

  it('offers Revivify only for a recently dead ally and restores it to life', () => {
    const encounter = new Encounter({ seed: 1 });
    encounter.addCreature({ heroClass: 'Cleric', heroLevel: 5, team: 'red', position: { x: 0, y: 0 }, heroOverrides: { additionalActions: [revivify('wis', 3, 3)], additionalResources: { 'slot-3': 1 } } });
    encounter.addCreature({ heroClass: 'Fighter', heroLevel: 5, team: 'red', position: { x: 1, y: 0 } });
    encounter.addCreature({ monster: 'Ogre', team: 'blue', position: { x: 5, y: 0 } });
    encounter.start();
    const caster = encounter.state!.creatures.find(creature => creature.monsterData.heroClass === 'Cleric')!;
    const ally = encounter.state!.creatures.find(creature => creature.monsterData.heroClass === 'Fighter')!;
    ally.isAlive = false;
    ally.currentHp = 0;
    ally.stats.deathRound = encounter.state!.round;
    encounter.state!.initiativeOrder = [caster.id];
    startArena(encounter);
    const action = getLegalActions(encounter, caster.id).find(choice => choice.type === 'spell' && choice.actionName === 'Revivify')!;
    expect(action.targetId).toBe(ally.id);
    applyLegalAction(encounter, action);
  ...
  it('selects and resolves each Scorching Ray independently', () => {
    ...
    encounter.state!.initiativeOrder = [caster.id];
    startArena(encounter);
    const rayChoice = getLegalActions(encounter, caster.id).find(action => action.type === 'spell' && action.actionName === 'Scorching Ray' && action.targetIds?.length === 3)!;
    applyLegalAction(encounter, rayChoice);
    expect(caster.stats.attacksMade).toBe(3);
  });

  it('binds Protection from Energy to a server-generated resistance choice', () => {
    const encounter = new Encounter({ seed: 1 });
    encounter.addCreature({ heroClass: 'Druid', heroLevel: 5, team: 'red', position: { x: 0, y: 0 }, heroOverrides: { additionalActions: [protectionFromEnergy('wis', 3, 3)], additionalResources: { 'slot-3': 1 } } });
    encounter.addCreature({ monster: 'Ogre', team: 'blue', position: { x: 2, y: 0 } });
    encounter.start();
    const caster = encounter.state!.creatures.find(creature => creature.team === 'red')!;
    encounter.state!.initiativeOrder = [caster.id];
    startArena(encounter);
    const action = getLegalActions(encounter, caster.id).find(choice => choice.type === 'spell' && choice.actionName === 'Protection from Energy' && choice.damageResistance === 'fire')!;
    applyLegalAction(encounter, action);
  ...
  it('makes attackers pass Sanctuary before they can deal damage', () => {
    const encounter = new Encounter({ seed: 1 });
    encounter.addCreature({ heroClass: 'Cleric', heroLevel: 5, team: 'red', position: { x: 0, y: 0 }, heroOverrides: { additionalActions: [sanctuary('wis', 3, 3)], additionalResources: { 'slot-1': 1 } } });
    encounter.addCreature({ monster: 'Ogre', team: 'blue', position: { x: 1, y: 0 } });
    encounter.start();
    const protectedCreature = encounter.state!.creatures.find(creature => creature.team === 'red')!;
    const attacker = encounter.state!.creatures.find(creature => creature.team === 'blue')!;
    const attack = attacker.monsterData.actions.find(action => action.type === 'melee' && action.attackBonus !== undefined)!;
    expect(executeSpell(encounter.state!, protectedCreature, sanctuary('wis', 3, 3), protectedCreature)).toBe(true);
    const hp = protectedCreature.currentHp;
  ...
  it('blocks a hostile single-target saving-throw spell when the caster fails Sanctuary', () => {
    const encounter = new Encounter({ seed: 1 });
    encounter.addCreature({ heroClass: 'Cleric', heroLevel: 5, team: 'red', position: { x: 0, y: 0 }, heroOverrides: { additionalActions: [sanctuary('wis', 3, 3)], additionalResources: { 'slot-1': 1 } } });
    encounter.addCreature({ heroClass: 'Cleric', heroLevel: 5, team: 'blue', position: { x: 2, y: 0 }, heroOverrides: { additionalActions: [command('wis', 3, 3)], additionalResources: { 'slot-1': 1 } } });
    encounter.start();
    const target = encounter.state!.creatures.find(creature => creature.team === 'red')!;
    const caster = encounter.state!.creatures.find(creature => creature.team === 'blue')!;
    expect(executeSpell(encounter.state!, target, sanctuary('wis', 3, 3), target)).toBe(true);
  ...
  it('gives Tasha\'s Hideous Laughter damage-triggered save advantage', () => {
    const encounter = new Encounter({ seed: 1 });
    encounter.addCreature({ heroClass: 'Wizard', heroLevel: 5, team: 'red', position: { x: 0, y: 0 }, heroOverrides: { additionalActions: [tashasHideousLaughter('int', 3, 3)], additionalResources: { 'slot-1': 1 } } });
    encounter.addCreature({ monster: 'Ogre', team: 'blue', position: { x: 2, y: 0 } });
    encounter.start();
    const caster = encounter.state!.creatures.find(creature => creature.team === 'red')!;
    const target = encounter.state!.creatures.find(creature => creature.team === 'blue')!;
    withRng({ next: () => 0 }, () => expect(executeSpell(encounter.state!, caster, tashasHideousLaughter('int', 3, 3), target)).toBe(true));
    applyDamage(encounter.state!, target, 1, 'force', caster);
    const rolls = [0, 0.99];
    withRng({ next: () => rolls.shift()! }, () => processTargetTurnEndOngoingEffects(encounter.state!, target));
    expect(target.activeBuffs.some(buff => buff.key === 'hideous-laughter')).toBe(false);
  });

  it('keeps Grease active for end-of-turn control saves', () => {
    const encounter = new Encounter({ seed: 1 });
    encounter.addCreature({ heroClass: 'Wizard', heroLevel: 5, team: 'red', position: { x: 0, y: 0 }, heroOverrides: { additionalActions: [grease('int', 3, 3)], additionalResources: { 'slot-1': 1 } } });
    encounter.addCreature({ monster: 'Ogre', team: 'blue', position: { x: 2, y: 0 } });
    encounter.start();
    const caster = encounter.state!.creatures.find(creature => creature.team === 'red')!;
    const target = encounter.state!.creatures.find(creature => creature.team === 'blue')!;
    withRng({ next: () => 0 }, () => expect(executeSpell(encounter.state!, caster, grease('int', 3, 3), target, [target], target.position)).toBe(true));
    expect(encounter.state!.persistentZones).toHaveLength(1);
    target.conditions = [];
  ...
  it('creates a movable Flaming Sphere concentration aura', () => {
    const encounter = new Encounter({ seed: 1 });
    encounter.addCreature({ heroClass: 'Wizard', heroLevel: 5, team: 'red', position: { x: 0, y: 0 }, heroOverrides: { additionalActions: [flamingSphere('int', 3, 3)], additionalResources: { 'slot-2': 1 } } });
    encounter.addCreature({ monster: 'Ogre', team: 'blue', position: { x: 2, y: 0 } });
    encounter.start();
    const caster = encounter.state!.creatures.find(creature => creature.team === 'red')!;
    const target = encounter.state!.creatures.find(creature => creature.team === 'blue')!;
    expect(executeSpell(encounter.state!, caster, flamingSphere('int', 3, 3), target, [target], target.position)).toBe(true);
  ...
  it('resolves Cloud of Daggers without a saving throw when a creature starts in it', () => {
    ...
    withRng({ next: () => 0 }, () => expect(executeSpell(encounter.state!, caster, cloudOfDaggers('int', 3, 3), target, [target], target.position)).toBe(true));
    expect(target.currentHp).toBe(initialHp);
    expect(caster.concentrationAura).toMatchObject({ spellName: 'Cloud of Daggers', automaticDamage: true, origin: 'point' });
    const hpBeforeTurn = target.currentHp;
    withRng({ next: () => 0 }, () => processTurnStart(encounter.state!, target));
    expect(target.currentHp).toBe(hpBeforeTurn - 4);
    expect(encounter.state!.events.some(event => event.kind === 'save' && event.targetId === target.id)).toBe(false);
  });
  ...
  it('repeats Call Lightning from authoritative concentration state without another slot', () => {
    ...
    const caster = encounter.state!.creatures.find(creature => creature.team === 'red')!;
    const target = encounter.state!.creatures.find(creature => creature.team === 'blue')!;
    expect(executeSpell(encounter.state!, caster, callLightning('wis', 3, 3), target, [target], target.position)).toBe(true);
    expect(caster.resources['slot-3']).toBe(0);
    encounter.state!.round++;
    encounter.state!.initiativeOrder = [caster.id];
    startArena(encounter);
    const repeat = getLegalActions(encounter, caster.id).find(action => action.type === 'repeat_area_spell')!;
    applyLegalAction(encounter, repeat);
    expect(caster.resources['slot-3']).toBe(0);
  ...
  it('resolves Dissonant Whispers flee through the target reaction', () => {
    const encounter = new Encounter({ seed: 1 });
    encounter.addCreature({ heroClass: 'Bard', heroLevel: 5, team: 'red', position: { x: 0, y: 0 }, heroOverrides: { additionalActions: [dissonantWhispers('cha', 3, 3)], additionalResources: { 'slot-1': 1 } } });
    encounter.addCreature({ monster: 'Ogre', team: 'blue', position: { x: 2, y: 0 } });
    encounter.start();
    const caster = encounter.state!.creatures.find(creature => creature.team === 'red')!;
    const target = encounter.state!.creatures.find(creature => creature.team === 'blue')!;
    withRng({ next: () => 0 }, () => expect(executeSpell(encounter.state!, caster, dissonantWhispers('cha', 3, 3), target)).toBe(true));
    expect(target.reactionUsed).toBe(true);
  ...
  it('offers Thunderwave to every SRD caster class', () => {
    for (const heroClass of ['Bard', 'Druid', 'Sorcerer', 'Wizard'] as const) expect(getAvailableSpells(heroClass, 5).some(action => action.name === 'Thunderwave')).toBe(true);
  });
