...
function parseAction(value: unknown): ArenaAction {
  ...
  if (action.id === 'move_aura') {
    if (!Number.isInteger(action.x) || !Number.isInteger(action.y)) throw new EncounterError('move_aura requires integer x and y.');
    return { id: 'move_aura', type: 'move_aura', destination: { x: action.x as number, y: action.y as number } };
  }
