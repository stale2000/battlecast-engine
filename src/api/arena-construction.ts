...
function highElfCantrip(name: 'Fire Bolt' | 'Ray of Frost', castingAbility: CastingAbility, abilities: Abilities): MonsterAction {
...
export function parseArenaParty(value: unknown, team: Team): AddCreatureOptions[] {
  ...
  return party.characters.map((value, index) => {
    ...
    if (elfLineage === 'High Elf' && selectedHighElfCantrip !== undefined && !['Prestidigitation', 'Fire Bolt', 'Ray of Frost'].includes(selectedHighElfCantrip)) throw new EncounterError(`${label}.highElfCantrip must be an engine-supported Wizard cantrip.`);
