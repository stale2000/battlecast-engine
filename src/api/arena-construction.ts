...
export function parseArenaParty(value: unknown, team: Team): AddCreatureOptions[] {
  ...
  return party.characters.map((value, index) => {
    ...
    if (species === 'Human' && !['Alert', 'Magic Initiate (Cleric)', 'Magic Initiate (Wizard)', 'Savage Attacker', 'Tavern Brawler', 'Tough'].includes(humanOriginFeat ?? '')) {
