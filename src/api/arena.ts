...
function applyArenaSpell(state: NonNullable<Encounter['state']>, active: Creature, activeActions: MonsterAction[], legal: Extract<ArenaAction, { type: 'spell' }>): void {
  ...
  const spell = baseSpell && legal.areaShape && baseSpell.savingThrow
    ...
    : baseSpell && legal.effectKey && baseSpell.dispelMagic
      ...
      : baseSpell && legal.damageType && baseSpell.damageTypeChoice
        ? { ...baseSpell, damageTypeChoice: { ...baseSpell.damageTypeChoice, selected: legal.damageType }, damageType: legal.damageType, buff: baseSpell.buff?.grantsAction ? { ...baseSpell.buff, grantsAction: { ...baseSpell.buff.grantsAction, damageType: legal.damageType, damageTypeChoice: { choices: baseSpell.buff.grantsAction.damageTypeChoice?.choices ?? [], selected: legal.damageType } } } : baseSpell.buff }
      : baseSpell && legal.sizeChange && baseSpell.sizeChangeChoice
        ? { ...baseSpell, sizeChangeChoice: { ...baseSpell.sizeChangeChoice, selected: legal.sizeChange }, buff: legal.sizeChange === 'enlarge' ? { name: 'Enlarge', key: 'enlarge-reduce', requiresConcentration: true, strengthTestAdvantage: true, weaponDamageBonusDice: '1d4' } : { name: 'Reduce', key: 'enlarge-reduce', requiresConcentration: true, strengthTestDisadvantage: true, weaponDamagePenaltyDice: '1d4' } }
      : baseSpell && legal.curseChoice ? withCurseChoice(baseSpell, legal.curseChoice) : baseSpell;
